...
class TestBrighterPlanetBilling < Test::Unit::TestCase  
  def test_001_count
    assert(::BrighterPlanet.billing.impact_estimate_service.queries.count > 1_000)
  end
  ...
  def test_011_immediate_store_to_mongo
    ...
    sleep 1
    stored_query = ::BrighterPlanet.billing.impact_estimate_service.queries.find_one(:execution_id => execution_id)
    TEST_TIME_ATTRS.each do |time_attr|
      assert_equal ::Time, stored_query.send(time_attr).class
    end
    assert_equal 'ImpactEstimateService', stored_query.service.name
  ...
  def test_012_delayed_store_to_mongo
    ...
    assert_nil ::BrighterPlanet.billing.impact_estimate_service.queries.find_one(:execution_id => execution_id)
